...
instance Show GameData where
  show (GameData width height board) =
    let heading = mconcat ["Board ", show width, " * ", show height]
        cells = do
          x <- 0 `to` height
...
setCellValue coord value =
  modify (\g -> g {gameBoard = M.insert coord value (gameBoard g)})

getCellValue :: Coordinate -> Game Cell
getCellValue coord = gets $ fromMaybe Dead . M.lookup coord . gameBoard
...
evolutionRules current neighbours =
  ...
  in case current of
       Dead
         | livingNeighbours == 3 -> Alive
       Dead -> Dead
       Alive
         | livingNeighbours < 2 -> Dead
       Alive
         | livingNeighbours > 3 -> Dead
       Alive -> Alive
