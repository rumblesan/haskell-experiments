...
runParser args (ArgParserFlag flagName flagFunc) =
  fmap flagFunc (M.lookup flagName args)
...
getArgParserFlags (ArgParserBranch l r) =
  (getArgParserFlags l) ++ (getArgParserFlags r)

data ZooKeeperConfig = ZooKeeperConfig
  { host :: String
  , port :: Int
  } deriving (Show)

data SomeOtherConfig = SomeOtherConfig
  { foo :: String
  , bar :: Int
  } deriving (Show)
