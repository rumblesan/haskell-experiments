...
data VMState = VMState { vmStack :: [Int] }
...
interpret program = eval $ view program
  ...
  eval (Push value :>>= p) (VMState stack)         = interpret (p ()) $ VMState (value:stack)
  eval (Pop        :>>= p) (VMState (v:stack))     = interpret (p v) $ VMState stack
  eval (Pop        :>>= _) (VMState [])            = error "not enough items"
  eval (Add        :>>= p) (VMState (v1:v2:stack)) = interpret (p ()) $ VMState ((v1+v2):stack)
