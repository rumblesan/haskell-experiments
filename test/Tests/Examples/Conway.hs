...
test_GOLRules =
  let
    ruleFunc = uncurry evolutionRules
    inputs   = [
      (Dead, []),
...
test_deadGrid =
  let
    ...
    output = execState (simulateBoardNTimes 10) input
  in
    assertEqual "" emptyGame output

test_glider :: Assertion
test_glider =
  let
    emptyGame = createGame 6 6
    input = execState (
      do
        setCellValue (3,1) Alive
